/*
 *  drivers/cpufreq/cpufreq_supercharger.c
 *
...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_supercharger
static
#endif
struct cpufreq_governor cpufreq_gov_supercharger = {
       .name                   = "supercharger",
...
static int supercharger_powersave_bias_setspeed(struct cpufreq_policy *policy,
...
static void supercharger_powersave_bias_init_cpu(int cpu)
{
...
static void supercharger_powersave_bias_init(void)
{
	int i;
	for_each_online_cpu(i) {
		supercharger_powersave_bias_init_cpu(i);
	}
}
...
/* cpufreq_supercharger Governor Tunables */
 ...
 * rate is 10 ms because the user needs immediate reaction from supercharger
...
skip_this_cpu:
		...
		supercharger_powersave_bias_init();
	} else {
		...
		for_each_online_cpu(cpu) {
			...
			if (dbs_info->cur_policy) {
				/* cpu using supercharger, cancel dbs timer */
				dbs_timer_exit(dbs_info);
				...
				mutex_lock(&dbs_info->timer_mutex);
				supercharger_powersave_bias_setspeed(
					dbs_info->cur_policy,
					NULL,
					input);
				mutex_unlock(&dbs_info->timer_mutex);
...
/* PATCH : SMART_UP */
	...
	for_each_cpu(j, policy->cpus) {
		...
		/*
		 * For the purpose of supercharger, waiting for disk IO is an
...
static void dbs_refresh_callback(struct work_struct *work)
{
	...
	policy = this_dbs_info->cur_policy;
	if (!policy) {
		/* CPU not using supercharger governor */
...
static int dbs_sync_thread(void *data)
{
	...
	while (1) {
		...
		if (policy->cur < src_freq) {
			/* cancel the next supercharger sample */
			...
			/* reschedule the next supercharger sample */
				   ...
				   unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		this_dbs_info->rate_mult = 1;
		supercharger_powersave_bias_init_cpu(cpu);
		/*
		...
		if (!supercharger_powersave_bias_setspeed(
...
static int __init cpufreq_gov_dbs_init(void)
{
	...
	dbs_wq = alloc_workqueue("supercharger_dbs_wq", WQ_HIGHPRI, 0);
	if (!dbs_wq) {
		printk(KERN_ERR "Failed to create supercharger_dbs_wq workqueue\n");
		return -EFAULT;
	}
	...
	return cpufreq_register_governor(&cpufreq_gov_supercharger);
}
...
static void __exit cpufreq_gov_dbs_exit(void)
{
	...
	cpufreq_unregister_governor(&cpufreq_gov_supercharger);
	for_each_possible_cpu(i) {
		struct cpu_dbs_info_s *this_dbs_info =
			&per_cpu(od_cpu_dbs_info, i);
		mutex_destroy(&this_dbs_info->timer_mutex);
...
MODULE_DESCRIPTION("'cpufreq_supercharger' - A dynamic cpufreq governor for "
	"Low Latency Frequency Transition capable processors");
MODULE_LICENSE("GPL");

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_supercharger
